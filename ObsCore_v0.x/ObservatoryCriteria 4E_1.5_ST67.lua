...
-- v1.5.0 / Released November 14, 2024
...
---------------------------------------- 
	...
	To get this criteria working with Elite Observatory:
		...
		- Click on "Observatory Explorer" section and then on "Plugin Settings"
...
LANDABLES, ATMOSPHERES AND SIGNALS
	...
	4.8: Landable with high angular diameter
 	4.9: Landable with high gravity
 	4.10: Large landable body
 	4.11: Ringed landable
...
-- 3.7: WIDE RING (TO DO)
-------------------------
--[[This notification can be triggered in two ways
		- RELATIVE: a ring which looks wide relative to the parent (based on the parent's radius)
		- OBJECTIVE: an objectively wide ring based on its own inner and outer radius
	You can enable either notification, or both, below, and configure their thresholds.
]]--

	triggerForRelativeWideRings = true
	triggerForObjectiveWideRings = true	
...
-- 4.5: LANDABLE AND CLOSE TO A STAR
------------------------------------
--[[These variables are used in the "Landable and close to a star" notification:
...
-----------------------------------------------------------------
-- 4.7: LANDABLE WITH ATMOSPHERE AND/OR BIOLOGICAL SIGNALS
-----------------------------------------------------------------
...
-- 4.8: LANDABLE WITH HIGH ANGULAR DIAMETER
-------------------------------------------
--[[This triggers for landable bodies which have a parent that looms large in the sky.]]--
...
--[[This triggers for any landable body with surface gravity greater than gravityThreshold (in G).]]--
	...
	gravityThreshold = 3 --number, in Gs, which will trigger the ping
...
-- 4.11: RINGED LANDABLE
-----------------------------------
--[[These criteria will tell you if any landable bodies you scan have rings around them. You can opt to 
	be notified about all of them, or just restrict the notification to only those with atmospheres.]]--

--Set this to true to be notified of landables with rings around them.
	triggerForRingedLandable = true
	
--Set this to true if you ONLY want the above notification to trip if the ringed landable has an atmosphere.
...
--This converts Pascals to atmospheres.
function pascalToAtm(pasc)
	return (pasc/101325)
end
...
if triggerForAncientStars == true then
	if scan.StarType and scan.StarType ~= '' then
		if scan.Age_MY >= starAgeThreshold then
			local luminosityResult = 'Luminosity missing'
...
if triggerForHeliumRichGasGiant == true then
    if scan.PlanetClass and string.match(scan.PlanetClass, 'Helium rich gas giant') then
        ...
        if scan.AtmosphereComposition then
            ...
            atmosComp = 'Atmosphere composition missing'
        end
        return true, 'Helium-rich gas giant' .. ringedHRGG, atmosComp
...
if triggerForHighValueBody == true then 
	...
	if highValueClasses[scan.PlanetClass] then
		...
		if (scan.TerraformState and #scan.TerraformState > 0 and scan.PlanetClass == "Metal rich body") then
			...
			isRare = string.char(10) .. "*RARE*"
		end
		return true, highValue .. isRare , scan.PlanetClass .. isTerraformable .. isLandable
	end
end
::End::
...
if triggerForProtostarTemps == true then
	if scan.StarType and scan.StarType ~= nil then
		if scan.StarType == 'TTS' or scan.StarType == 'AeBe' then
			local realTemp = tonumber(string.format('%.6f', scan.SurfaceTemperature)) -- Sadly required to kinda fix Observatory's faulty data...
            local protoLumi = scan.Luminosity
            if not protoLumi then -- Sometimes really old log files don't have luminosity data so we need a string for the output to avoid errors.
                protoLumi = "Luminosity missing"
            end			
			if has_value(StrangeProtostarTemperatures[scan.StarType], realTemp) then
				...
				.. string.char(10) .. 'Spectral class: ' .. scan.StarType .. scan.Subclass .. ' ' .. protoLumi
			end
		end
	end
end
		...
		for ring in rings(scan.Rings) do
			if not (string.find(ring.name, "Belt")) then
				if math.floor(ring.massmt) >= (ringMassThreshold * 1000000000000) and largeRingTypes[ring.ringclass] then
					...
					result = ring.name .. string.char(10) .. "Ring type: " .. ringType .. " // Mass: " .. formatNumber(ringMass) .. " Gt" .. string.char(10) .. "Diameter: " .. formatNumber(ringDiameter) .. " LS // Width: " .. formatNumber(ringWidth) .. " LS"
					return true, "Massive ring" .. string.char(10) .. string.char(10), result
				end
			end
		end
	end
		...
		if parents[0].Scan and parents[0].Scan.Rings then --check for parents with rings
			...
			if hasBelt == false then --continues the function if the rings detected are, in fact, rings
				...
				if (moonOrbit - outerRadius[ringCount]) < 0 and triggerForShepherdMoon == true then
					local innerRingDistance = ''
					if (ringCount > 1) then
						innerRingDistance = 'Distance to inner ring: ' .. formatNumber(math.floor(moonOrbit - outerRadius[ringCount - 1])) ..' km // Orbit: ' .. formatNumber(moonOrbit) .. ' km' .. string.char(10)
					end
					return true, 'Shepherd moon' .. string.char(10) .. canLand, innerRingDistance .. 'Distance to outer ring: ' .. formatNumber(math.floor(innerRadius[ringCount] - moonOrbit)) .. ' km'
				end
				...
				if math.abs((moonOrbit - outerRadius[ringCount])) < closeRingDistance then -- subtract the outer radius of the outermost ring from the orbit of the moon,
					return true, 'Close ring proximity' .. canLand .. string.char(10) ..
	...
	if hasRings(scan.Rings) then
		...
		for ring in rings(scan.Rings) do
			...
			if periodResult ~= "" then
				return true, "Ring has fast orbital period" .. string.char(10) .. "(" .. formattedTime .. ")", periodResult
			end
		end
	end
end
		...
		for ring in rings(scan.Rings) do
			if not (string.find(ring.name, "Belt")) then
				...
				if orbitalVelocity >= ringVelocityThreshold then
					velocityResult = velocityResult .. "Ring: " .. ring.name
				end
				if velocityResult ~= "" then
					return true, "Ring has high orbital velocity" .. string.char(10) .. "(" .. Round(orbitalVelocity,2) .. " km/s)", velocityResult
				end
			end
		end
	end
...
-- 3.7: WIDE RING (TO DO)
-------------------------
--Wide ring (relative) - a ring which looks wide relative to its parent
--Wide ring (objective) - a ring classified as wide based on its inner and outer radius
...
::Criteria::
if triggerForHighSurfacePressure == true then
	if scan.Landable and scan.Atmosphere and pascalToAtm(scan.SurfacePressure) >= atmoSatThreshold then
		return true, "High surface pressure (Landable)" .. string.char(10) .. "(" .. Round(pascalToAtm(scan.SurfacePressure),3) .. " atm.)", "Atmosphere type: " .. scan.AtmosphereType
	end
end
::End::
...
--------------------------------------------------------------------------------
-- 4.7: LANDABLE WITH ATMOSPHERE AND BIOLOGICAL SIGNALS / 4.11: RINGED LANDABLES
--------------------------------------------------------------------------------
::Criteria::
...
if triggerForRingedLandable == true or triggerForLandableAtmosphere == true or triggerForBiosignals == true then
...
-- building the ringed prefix if necessary
	if scan.Landable and scan.Rings and triggerForRingedLandable == true then
		baseText = "Ringed landable"
		if RingedWithAtmosphereOnly == false then
			notify = true
		end
	end

-- appending atmosphere
	if scan.Landable and scan.Atmosphere ~= "" then
		withAtmo = " with " .. scan.Atmosphere
		if (triggerForLandableAtmosphere == true and (Included_Atmospheres[scan.AtmosphereType] or isNewAtmosphere(scan.AtmosphereType))) or scan.Rings and RingedWithAtmosphereOnly == true then
			notify = true
		end
	end
	
-- appending biological signals
	if biosignals > 0 and triggerForBiosignals == true then
		withLife = " and " .. string.format('%i biological signal%s', biosignals, biosignals > 1 and 's' or '')
		if triggerForBiosignals == true then
			notify = true
		end
	end
...
-- write a note if one of certain exceptional things is true
		...
		elseif scan.Atmosphere == '' and biosignals > 0 and triggerForBiosignals == true then
			notes = string.char(10) .. "(Horizons bios likely)"
		end
		
		return true, baseText .. withAtmo .. withLife .. notes, "Body type: " .. scan.PlanetClass
	end
end
::End::
...
-------------------------------------------
::Criteria::
if triggerForHighAngularDiameter == true then
...
if triggerForHighG == true then
	...
	if gravityCalc >= 2.7 then
		onFootProhibited = string.char(10) .. "SRV disembarkment only"
	end
	if scan.Landable and gravityCalc >= gravityThreshold then
		return true, "Landable with high gravity" .. string.char(10) ..	"(" .. Round(gravityCalc,2) .. " G)" .. onFootProhibited, "Body type: " .. scan.PlanetClass .. string.char(10) .. "Surface temperature: " .. math.floor(scan.SurfaceTemperature) .. " K"
	end
end
::End::
...
--[[4.11 is combined with 4.7 above]]-- 



----------------------------------------
-- Cmdr SpaceTrash67 Addtiional Criteria
----------------------------------------
